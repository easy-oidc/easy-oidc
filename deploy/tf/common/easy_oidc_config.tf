  ...
  validation {
    condition = !(
      ...
      (contains([for connector in values(var.easy_oidc_config.user_login_connectors) : connector.type != "email"], true) && (
        contains([for client in values(coalesce(try(var.easy_oidc_config.static_policy.clients, null), {})) : try(coalesce(client.refresh_tokens.enabled, false), false)], true) ||
        try(coalesce(var.easy_oidc_config.policy_database.client_defaults.refresh_tokens.enabled, false), false)
      ))
    ) || try(trimspace(var.easy_oidc_config.secrets.encryption_key_name) != "", false)
    error_message = "easy_oidc_config.secrets.encryption_key_name is required for GitHub and for enabled refresh tokens used with a non-email connector."
  }
