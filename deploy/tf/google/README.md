...
OpenTofu/Terraform module for deploying [Easy OIDC](https://github.com/easy-oidc/easy-oidc) on Google Cloud.

The module provisions a single Compute Engine instance, Caddy with automatic TLS, optional dual-stack networking, local SQLite storage, and Google Secret Manager access. Application settings are supplied directly as an Easy OIDC configuration object.
...
```bash
openssl rand -hex 32 | \
  gcloud secrets create easy-oidc-encryption-key \
    --replication-policy=automatic \
    --data-file=-
```
...
access to that secret and run `easy-oidc migrate` before every service start. A
failed migration prevents Easy OIDC from starting. Because migration
...
module "easy_oidc" {
  ...
  easy_oidc_config = {
    secrets = {
      signing_key_name    = "projects/my-project/secrets/easy-oidc-signing-key/versions/latest"
      encryption_key_name = "projects/my-project/secrets/easy-oidc-encryption-key/versions/latest"
    }
    user_login_connectors = {
      google = {
        type               = "google"
        display_name       = "Google"
        credentials_secret = "projects/my-project/secrets/easy-oidc-google/versions/latest"
      }
    }
    static_policy = {
      user_group_mappings = {
...
Configure the API server with the module's issuer and a key from `easy_oidc_config.static_policy.clients` (or a client ID managed by `policy_database`):
...
| `easy_oidc_config` | Typed Easy OIDC v2 configuration object; deployment-owned fields are overridden | `object` | — | yes |
...
| `easy_oidc_version` | Easy OIDC release; must be `v2.0.0` or later, or `latest` | `string` | `"latest"` | no |
...
| `grant_secret_accessor` | Grant the instance service account access to each Secret Manager secret referenced by `easy_oidc_config` | `bool` | `true` | no |
...
| `client_ids` | Keys from `easy_oidc_config.static_policy.clients`; empty for policy-database-only clients |
...
| `easy_oidc_version` | Resolved Easy OIDC version |
