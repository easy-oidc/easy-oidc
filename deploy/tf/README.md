...
For distribution/publishing, Truster OpenTofu/Terraform modules can be found at:

- https://github.com/truster-dev/terraform-aws-truster for AWS
- https://github.com/truster-dev/terraform-google-truster for Google
...
```sh
scripts/generate-tf-modules.sh /tmp/truster-tf
```
...
`truster-dev/terraform-aws-truster` and
`truster-dev/terraform-google-truster`. Configure a GitHub App with Contents
...
changed module is committed to `main` and receives the same tag as the Truster
release; its branch and tag are pushed atomically. Before either repository
is changed, every changed module is checked for a conflicting tag. Module tags
therefore record the Truster release that published that module revision
